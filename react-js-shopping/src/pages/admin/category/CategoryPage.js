...
const CategoryPage = () => {
    ...
    useEffect(() => {
        (
            async () => {
                const res = await fn_getCategoryData();
    ...
    const fn_getCategoryData = async () => {
        try {
    ...
    const fn_deleteCategoryById = async (categoryId) => {
        try {
    ...
    useEffect(() => {
        setTotalPages(Math.ceil(allCategories.length / chooseNoOfRowsToShow))
        fn_getPaginatedData(allCategories, chooseNoOfRowsToShow)
    }, [chooseNoOfRowsToShow, currentPage, allCategories])
        ...
        <div className='container'>
                                ...
                                return (
                                    <tr key={index}>
                                        ...
                                        <td style={{ maxWidth: "3rem" }}>
                                            <img src={`http://${process.env.REACT_APP_SERVER_IP}:${process.env.REACT_APP_SERVER_PORT}/${item.categoryImage}`} style={{ width: '60px', height: '60px', alignSelf: 'center' }} alt='product' />
                                        </td>
                                        <td>
                                            <div className="btn-group" role="group" aria-label="Basic example">
                                                <button type="button"
                                                    ...
                                                    onClick={async () => {
                                                        const res = await fn_deleteCategoryById(item._id);
                                                        if (res.success) {
                                                            const resCategory = await fn_getCategoryData();
                                                            console.log('resCategory', resCategory)
                                                            setAllCategories(resCategory.data)
                                                        } else {
                                                            console.log('res', res)
                                                        }
                                                    }}
            ...
            <div className={styles.fbc}>
                ...
                <div className={styles.fc}>
                    <p className='mx-5'>Showing {chooseNoOfRowsToShow * currentPage + 1 - chooseNoOfRowsToShow} - {currentPage * chooseNoOfRowsToShow} of {allCategories.length}  entries  </p>
                    <div className="btn-group" role="group" aria-label="Basic example">
